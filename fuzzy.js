...
function fuzzyCheck (searchTerm, testName, library) { //report score for test string @ test card and current best card
	...
	if(library.name == "revolution"){
		needleScript = function(card) {
			if(library.legal.rotated.includes(card.setID))
				return -3;
			return 0;
		}
	}
	let testScore = fuzzySearch(searchTerm, testName, 0);//, true); //debugging purposes
	let current = searchCards(library, searchTerm, needleScript);
	let currentScore = fuzzySearch(searchTerm, current, needleScript(library.cards[current]));
...
function fuzzySearch (needle, thread, needlescore, showMe) { //beta scoring function for the search
	let charScore = 1; stringBonus = 0.5; threadOffset = 0, needleOffset = 0;
	needle = needle.toLowerCase();
	thread = thread.toLowerCase();
	if(needle == thread)
		return needlescore+(charScore+stringBonus)*needle.length;
	...
	boxLine: for(var h = 0; h < needleBox.length; h++) { 			//for each word in the searchstring
		let newstart = 0;
		if(h+needleOffset >= needleBox.length)
			return needlescore;
		needleLine: for (var i = 0; i < needleBox[h+needleOffset].length; i++) { //for each character in the search string
			...
			if(i == 0) {//try to match the first character
				...
				while(h+tempOffset < threadBox.length){				//check the remaining words to match first characters
					if(threadBox[h+tempOffset].charAt(i) != needleBox[h+needleOffset].charAt(i)) { //if this word doesn't start with it
						if(showMe) {
							if(threadBox[h+tempOffset+1]) {
								console.log("try " + threadBox[h+tempOffset+1])
							}else{
								console.log("out of thread")
							}
						}
				...
				if(h+tempOffset != threadBox.length) { //if something matched the first character
					...
					let tempOffset = 0;
					tempOffset += needleOffset; //seperate so it's not a js reference
					while(h+needleOffset < needleBox.length){
						if(needleBox[h+tempOffset] && !needleBox[h+tempOffset].match(escapify(threadBox[h+threadOffset].charAt(0)))) { //if this word doesn't have it
							if(showMe) {
								if(needleBox[h+tempOffset+1]) {
									console.log("try " + needleBox[h+tempOffset+1])
								}else{
									console.log("out of needle")
								}
							}
							tempOffset++; //try the next one
						}else{
					...
					if(h+tempOffset != needleBox.length) //if something matched the first character
						needleOffset = tempOffset; //set threadOffset to the right word;
				}
			...
			for (var j = newstart; j < threadBox[h+threadOffset].length; j++){ //for the characters in the checking string
				if(threadBox[h+threadOffset].charCodeAt(j) === needleBox[h+needleOffset].charCodeAt(i)){
					needlescore += charScore;
					if(showMe)
						console.log("needlescore to " + needlescore + " at " + needleBox[h+needleOffset].charAt(i) + " matching " + threadBox[h+threadOffset].charAt(j));
					if(j == newstart) {
...
function fuzzySearchSimple (needle,thread,needlescore) { //scoring function for the search
	...
	if(needle == thread)
		return (charScore+stringBonus)*needle.length;
	if(thread.length > 65) { //for very long names, reduce the score
		charScore = 0.5; stringBonus = 1;
	}
...
function searchCards(library,searchstring,needleWeight,scoreObj,skip) {  //main search function
	let database = library.cards;
	...
	for(let entry in database) {
		...
		if(splitString[2] !== null && splitString[2] !== undefined) {
			if(splitString[2].toUpperCase() == database[entry].setID)
				needlescore = 4;
		...
		if(database[entry].hasOwnProperty('hidden'))
			names.push(database[entry].hidden.replace(/-/g," "));
		if(database[entry].hasOwnProperty('alias'))
			names.push(database[entry].alias.replace(/-/g," "));
		if(database[entry].hasOwnProperty('cardName2')) {
			names.push(database[entry].cardName2.replace(/-/g," "));
			names.push(database[entry].cardName.replace(/-/g," "));
		}
		//let shout = (entry == "Mind-Mute_DHL");
		for(let aName in names) {
			if(!(splitString[3] && !splitString[1])) //if not only a filter, fuzzy search
				...
				/*if(shout) {
					...
					console.log(names[aName]);
					minPercent = 0.8;
				}*/
			if(minPercent) {
				let p = (1.5*splitString[1].length); // max score
				if(p !== 0)
					p = i / p; //percentage	
				//if(shout)
				//	console.log(p);
...
var tagFunctions = {
		funny: function(library)  {// is:funny
		...
		permanent: function()  {// is:permanent
			return function(card) {
		...
		transform: function()  {// is:transform
			return function(card) {
		...
		useless: function()  { //is:useless island
			return function(card) {
		...
		scuttleback: function()  { //is:scuttleback
			return function(card) {
				if(card.typeLine.match(/\b(Crab|Fish|Jellyfish|Kraken|Leviathan|Merfolk|Nautilus|Octopus|Oyster|Pirate|Seal|Serpent|Shark|Shrimp|Sponge|Squid|Starfish|Trilobite|Turtle|Whale|Island)\b/))
					return true;
				return false;
			};
		},
		tale: function()  { //is:scuttleback
			return function(card) {
...
function generateScryCode (thisCheck, library, flags) { //makes the function for individual scryfall arguments
	...
	if(!operMatch.match(":") && (thisCheck.match(/cmc/) || !thisCheck.match(/c(:| ?[<>=]{1,2})/i))) { //rarity and power/toughness/loyalty/cmc
		...
		let formatMatch = matchCheck.match(/(vintage|legacy|commander|pauper|modern|pioneer|standard|brawl|historic|h|v|type1.5|type1|type2|edh|pi|p|1.5|1|2|msem2?|revolution|myriad)$/);
		if(formatMatch) {
			...
			if(formats[formatMatch]) {
				return function(card) {
					if(card.formats && card.formats.includes(formats[formatMatch]))
						return true;
					return false;
				};
			}
		...
		if(tagFunctions.hasOwnProperty(matchCheck.toLowerCase())) {
			return tagFunctions[matchCheck.toLowerCase()](library)
		}
